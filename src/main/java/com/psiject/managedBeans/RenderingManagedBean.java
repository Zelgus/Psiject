...
public class RenderingManagedBean implements Serializable {
	...
	public Boolean botonComentarPaciente(final Tarea tarea) {
		...
		Expediente expedienteSeleccionado = this.sessionMB
				.getExpedienteSeleccionado();
		try {
			if (usuarioEnSesion != null
					&& expedienteSeleccionado != null
					&& tarea != null
					&& !usuarioEnSesion.getPermiso()
					&& !expedienteSeleccionado.getCerrado()
					&& !tarea.getCompletada()
					&& !tareaCampoService.obtenerCamposPorTarea(tarea)
							.isEmpty()
	...
	public Boolean getBotonComentarPaciente() {
		...
		try {
			if (usuarioEnSesion != null
					...
					&& !usuarioEnSesion.getPermiso()
					&& !expedienteSeleccionado.getCerrado()
					&& !tareaSeleccionada.getCompletada()
					&& !tareaCampoService.obtenerCamposPorTarea(
							tareaSeleccionada).isEmpty()
					&& tareaSeleccionada.getExpediente().equals(
	...
	public Boolean getBarraNavegacionWizard() {
		Boolean res = false;
		final Usuario usuarioEnSesion = this.sessionMB.getUsuarioEnSesion();
		final Expediente expedienteSeleccionado = this.sessionMB
				.getExpedienteSeleccionado();
		...
		try {
			if (usuarioEnSesion != null
					...
					&& this.expedientePsicologoService
							.obtenerPsicologosConAccesoPorExpediente(
									expedienteSeleccionado).contains(
									usuarioEnSesion) && !tarea.getCompletada()
					&& tarea.getExpediente().equals(expedienteSeleccionado)) {
				res = true;
			}
		} catch (Exception e) {
	...
	public Boolean getBotonEditarComentario() {
		...
		Expediente expedienteSeleccionado = this.sessionMB
				.getExpedienteSeleccionado();
		Tarea tareaSeleccionada = this.sessionMB.getTareaSeleccionada();
		Comentario comentarioSeleccionado = this.sessionMB
		...
		if (usuarioEnSesion != null
				...
				&& tareaSeleccionada.getExpediente().equals(
						expedienteSeleccionado)
				&& comentarioSeleccionado.getUsuarioCreador().equals(
						usuarioEnSesion)) {
			res = true;
		}
		return res;
	...
	public Boolean botonEditarComentario(Comentario comentario) {
		...
		if (usuarioEnSesion != null
				...
				&& tareaSeleccionada.getExpediente().equals(
						expedienteSeleccionado)
				&& comentario.getUsuarioCreador().equals(usuarioEnSesion)) {
			res = true;
		}
		return res;
	}
	...
	public Boolean getBotonEditarExpediente() {
		...
		try {
			if (usuarioEnSesion != null
					&& expedienteSeleccionado != null
					&& usuarioEnSesion.getPermiso()
					&& expedienteSeleccionado.getUsuarioCreador().equals(
							usuarioEnSesion)
					&& this.expedientePsicologoService
							.obtenerPsicologosConAccesoPorExpediente(
									expedienteSeleccionado).contains(
	...
	public Boolean botonEditarExpediente(final Expediente expediente) {
		...
		try {
			if (usuarioEnSesion != null
					&& expediente != null
					&& usuarioEnSesion.getPermiso()
					&& expediente.getUsuarioCreador().equals(usuarioEnSesion)
					&& this.expedientePsicologoService
							.obtenerPsicologosConAccesoPorExpediente(expediente)
							.contains(usuarioEnSesion)
					&& !expediente.getCerrado()) {
				res = true;
			}
		} catch (Exception e) {
			Mensajes.mostrarLog(e);
			...
			if (usuarioEnSesion != null
					...
					&& this.expedientePsicologoService
							.obtenerPsicologosConAccesoPorExpediente(
									...
									usuarioEnSesion)
					&& !expedienteSeleccionado.getCerrado()
					&& tarea.getUsuarioCreador().equals(usuarioEnSesion)
					&& !tarea.getCompletada()
					&& tarea.getExpediente().equals(expedienteSeleccionado)) {
				res = true;
			}
			...
			if (usuarioEnSesion != null
					...
					&& this.expedientePsicologoService
							.obtenerPsicologosConAccesoPorExpediente(
									...
									usuarioEnSesion)
					&& !expedienteSeleccionado.getCerrado()
					&& tareaSeleccionada.getUsuarioCreador().equals(
							usuarioEnSesion)
					&& !tareaSeleccionada.getCompletada()
					&& tareaSeleccionada.getExpediente().equals(
							expedienteSeleccionado)) {
	...
	public Boolean botonConsultarTareaPaciente(final Tarea tarea) {
		...
		final Expediente expedienteSeleccionado = this.sessionMB
				.getExpedienteSeleccionado();
		if (usuarioEnSesion != null && expedienteSeleccionado != null
				&& tarea != null && !usuarioEnSesion.getPermiso()
				&& tarea.getExpediente().equals(expedienteSeleccionado)) {
			res = true;
		}
		return res;
	...
	public Boolean iconoNuevoComentarioDePacienteEnExpediente(
			final Expediente expediente) {
		Boolean res = false;
		final Usuario usuarioEnSesion = this.sessionMB.getUsuarioEnSesion();
		try {
			if (usuarioEnSesion != null && expediente != null
					&& usuarioEnSesion.getPermiso()) {
				final List<Tarea> tareas = this.tareaService
						.obtenerTareasPorExpediente(expediente);
				for (Tarea tarea : tareas) {
					if (tarea != null
							&& !tarea.getCompletada()
							&& tarea.getExpediente().equals(expediente)
							&& !this.tareaCampoService.obtenerCamposPorTarea(
									tarea).isEmpty()
							&& this.comentarioService
									.tareaConNuevoComentarioDePaciente(tarea)) {
						res = true;
						break;
					}
				}
	...
	public Boolean iconoNuevoComentarioDePaciente(final Tarea tarea) {
		...
		try {
			if (usuarioEnSesion != null
					...
					&& !tarea.getCompletada()
					&& tarea.getExpediente().equals(expedienteSeleccionado)
					&& !this.tareaCampoService.obtenerCamposPorTarea(tarea)
							.isEmpty()
					&& this.comentarioService
							.tareaConNuevoComentarioDePaciente(tarea)) {
				res = true;
		...
		try {
			if (usuarioEnSesion != null
					...
					&& !this.tareaCampoService.obtenerCamposPorTarea(tarea)
							.isEmpty()
					&& this.comentarioService
							.tareaConNuevoComentarioDePsicologo(tarea)) {
				res = true;
			}
		} catch (Exception e) {
			Mensajes.mostrarLog(e);
